...
### OWASP十大漏洞
1.  SQLI(SQL注入攻击)
    *   普通注入：一直数据库提示的有价值错误信息；
    *   盲注（Blind SQL injection）：有经验的管理员根据错误页面并不详细的错误信息运用脚本来通过仅有的判断信息（如时间差）对表中的每个字段进行探测，从而进行注入。
    >   盲注难度较大
2.  XSS
    *   存储型、反射型以及DOM型。
3.  CSRF
4.  回话认证管理缺陷（Broken Authentication and Session Management,BASM）
